...
const SectionTitle = ({ children }) => (
    <h2 className='text-xl font-bold text-ink dark:text-darkText border-b border-line dark:border-darkBorder pb-2 mt-12'>
        {children}
    </h2>
);
...
export default function ResumePage() {
    return (
        ...
        <main className='px-[8%] md:px-[12%] pt-28 pb-20 max-w-4xl mx-auto min-h-screen dark:text-darkText print:pt-8'>
            <Link
                href='/'
                className='inline-flex items-center gap-1 text-sm text-inkMuted hover:text-ink dark:text-darkMuted dark:hover:text-darkText print:hidden'
            >
                ← 홈으로
            </Link>

            <header className='mt-6'>
                <h1 className='text-4xl font-bold'>김윤성</h1>
                <ul className='flex flex-wrap gap-x-4 gap-y-1 mt-3 text-sm text-ink/80 dark:text-darkText/80'>
                ...
                <a
                    ...
                    className='inline-flex items-center gap-2 text-sm mt-5 border border-line bg-panel rounded-md px-4 py-2 w-fit hover:bg-pageBg dark:bg-darkSurface dark:border-darkBorder dark:hover:bg-darkHover print:hidden'
                >
            ...
            <p className='mt-3 text-ink/80 dark:text-darkText/80 leading-8'>
                모르는 문제를 조사하여 직접 풀어 나가며, IDS 탐지 환경을 직접 구축하고 악성코드를 정적·동적·네트워크로 분석해
                IOC를 도출하고 탐지 룰로 검증하는 흐름을 단독으로 끝까지 수행했습니다.
            </p>
            <p className='mt-2 text-ink/80 dark:text-darkText/80 leading-8'>
                    ...
                    <div key={area} className='sm:grid sm:grid-cols-[180px_1fr] sm:gap-4'>
                        ...
                        <dd className='mt-1 sm:mt-0'>
                            <ul className='space-y-1'>
                                {items.map((item, i) => (
                                    <li key={i} className='text-sm text-ink/80 dark:text-darkText/80 leading-7'>{item}</li>
                                ))}
                            </ul>
                        </dd>
                    </div>
            ...
            <div className='mt-4 space-y-10'>
                {projects.map(({ title, meta, bullets, detailLink }) => (
                    <section key={title}>
                        ...
                        <p className='font-mono text-xs text-inkMuted dark:text-darkMuted mt-1'>{meta}</p>
                        <ul className='mt-3 space-y-2'>
                            {bullets.map((item, i) => (
                                <li key={i} className='text-sm text-ink/80 dark:text-darkText/80 leading-7 flex items-start gap-2'>
                        ...
                        {detailLink && (
                            <Link
                                href={detailLink}
                                className='inline-flex items-center gap-1 text-sm mt-3 text-inkMuted hover:text-ink dark:text-darkMuted dark:hover:text-darkText underline underline-offset-4 print:hidden'
                            >
                                케이스 스터디 자세히 보기
                ...
                {reports.map(({ label, href }) => (
                    <li key={href}>
                        <a
                            ...
                            className='inline-flex items-center gap-2 text-sm text-ink/80 dark:text-darkText/80 hover:underline'
                        >
            ...
            <section className='mt-4'>
                ...
                <p className='font-mono text-xs text-inkMuted dark:text-darkMuted mt-1'>2023.11 ~ 2024.11 (1년) · 졸업 유예 기간 중 근무 시작</p>
                <p className='mt-3 text-sm text-ink/80 dark:text-darkText/80 leading-7'>
            ...
            <dl className='mt-4 space-y-3'>
                {[
                    ...
                    <div key={term} className='sm:grid sm:grid-cols-[180px_1fr] sm:gap-4'>
                        ...
                        <dd className='mt-1 sm:mt-0 text-sm text-ink/80 dark:text-darkText/80 leading-7'>{desc}</dd>
                    </div>
                ))}
            </dl>
        </main>
