...
// 문단 안에서 백틱(`)으로 감싼 부분은 인라인 코드(모노스페이스)로 렌더링된다.
// 섹션의 icon은 page.js의 lucide 아이콘 매핑 키, variant: 'callout'은 파란 강조 박스 표시.
    ...
    'snort-ids-cean-agenttesla': {
        ...
        sections: [
            ...
            },
            {
                ...
                icon: 'ServerCog',
                body: [
                    'VMware 기반의 격리된 Windows 11 환경에, Snort가 탐지한 경보를 MySQL에 저장하고 BASE가 이를 읽어 웹 대시보드로 보여주는 구조를 구축했다. `Snort 2.9.2.3` 설치·ICMP 탐지 확인 → `XAMPP 1.7.1`(Apache·PHP·MySQL) 설치 → snort DB·스키마 생성 → `snort.conf`에 output database 설정 → `ADOdb 5.20.21` 배치 → `BASE 1.4.5` 연결 순서다.',
            ...
            },
            {
                ...
                body: [
                    ...
                    '`sid:1000007`은 CEaN.exe가 ip-api[.]com에 분석 환경 여부를 묻던 정찰 패킷 기반 룰이다. `flow:established,to_server`로 연결 수립 후 클라이언트→서버 방향 패킷만 검사해 오탐을 줄이고, `http_uri`로 이 검체만 쓰는 특정 요청 경로를 URI 영역에 한정해 매칭한다. `nocase`는 대소문자 우회를 방지한다.',
                    '검증은 BASE 대시보드 준비 → `snort -W`로 캡처 인터페이스 확인 → IDS 모드 실행(`-k none`) → 사전 등록한 ICMP 테스트 룰로 탐지 파이프라인 정상 동작 점검 → 검체 실행 순으로 진행했다. 실행 결과 ip-api 정찰(sid:1000007), 악성 도메인 DNS 질의(sid:1000005·1000006), SMTP 발신·수신 주소(sid:1000008·1000009), 유출 본문(sid:1000011) 등이 Snort 실시간 로그와 BASE 대시보드에 탐지·출력되어, 정찰부터 유출까지 악성 행위의 각 단계가 작성한 룰에 포착됨을 검증했다.',
                ],
            },
            {
                ...
                variant: 'callout',
                body: [
                    '룰을 `local.rules`에 저장하고 검증했으나 여러 번 시도해도 탐지되지 않았다. 원인을 단계적으로 배제하며 좁혔다. ① Wireshark 캡처로 악성코드가 여전히 통신 중임을 확인해 트래픽 부재 가능성을 배제하고 ② `snort -W` 재확인으로 인터페이스 번호 오지정을 정정했으나 여전히 미탐지, ③ ICMP 테스트 룰(sid:1000099)이 정상 동작해 룰 문법과 Snort 엔진 파이프라인에는 문제가 없음을 확인했다.',
